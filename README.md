...
本番環境（サーバー）にデプロイする場合は、以下の点に注意してください：

- 環境変数（`DISCORD_TOKEN`）は、デプロイ先の環境変数設定機能を使用してください
- プロセス管理ツール（systemd、PM2、supervisorなど）を使用してBotを常時起動状態に保つことを推奨します
- ログファイル（`bot_translation.log`）のローテーション設定を推奨します
- クラウドホスティングサービス（Railway、Render、Herokuなど）を使用する場合は、各サービスの環境変数設定方法に従ってください
...
bolcofのサーバーを使ってbotを入れて、「応答がない」というメッセージが表示された場合、[GitHubのIssues](https://github.com/bolcof/DiscordTranslationBot/issues)で問題を報告してください。
